...
const ProductCardAdmin = ({ data, fetchProductData }) => {
  ...
  return (
    <div className='w-36 p-4 bg-white rounded'>
        ...
        <p className='text-ellipsis  line-clamp-2 font-medium whitespace-nowrap overflow-hidden'>{data?.bookName}</p>
        
        <div className='grid grid-cols-2 gap-3 py-2'>
          <button onClick={()=>setEditOpen(true)} className='border px-1 py-1 text-sm border-green-600 bg-green-100 text-green-800 hover:bg-green-200 rounded'><MdEdit size={24} className='items-center ml-1' /></button>
          <button onClick={()=>setOpenDelete(true)} className='border px-1 py-1 text-sm border-red-600 bg-red-100 text-red-600 hover:bg-red-200 rounded'><MdDelete size={24} className='items-center ml-1'/></button>
        </div>

        {
          editOpen && (
            <EditProductAdmin   />
            // fetchProductData={fetchProductData} data={data} close={()=>setEditOpen(false)}
